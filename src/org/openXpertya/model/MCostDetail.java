...
import ar.com.geneos.mrp.plugin.model.LP_M_CostDetail;
 ...
 *
 * @author Jorg Janke
 * @author Armen Rizal, Goodwill Consulting <li>BF: 2431123 Return Trx changes
 *         weighted average cost <li>BF: 1568752 Average invoice costing: landed
 *         costs incorrectly applied
 * @author Armen Rizal & Bayu Cahya <li>BF [ 2129781 ] Cost Detail not created
 *         properly for multi acc schema
 ...
 * 
 */
public class MCostDetail extends LP_M_CostDetail {
	/**
	 * 
	 */
	...
	private static final String COLUMNNAME_M_CostDetail_ID = "M_CostDetail_ID";
	...
	public static BigDecimal getCostByModel(int accountSchemaId, int costTypeId, int costElementId, IDocumentLine model) {
		...
		whereClause.append(LP_M_CostDetail.COLUMNNAME_C_AcctSchema_ID).append("=? AND ");
		whereClause.append(LP_M_CostDetail.COLUMNNAME_M_CostType_ID).append("=? AND ");
		whereClause.append(LP_M_CostDetail.COLUMNNAME_M_CostElement_ID).append("=? AND ");
		whereClause.append(CostEngine.getIDColumnName(model)).append("=? ");

		return new Query(model.getCtx(), LP_M_CostDetail.Table_Name, whereClause.toString(), model.get_TrxName()).setClient_ID()
				.setParameters(accountSchemaId, costTypeId, costElementId, model.get_ID())
				.sum("(" + MCostDetail.COLUMNNAME_Amt + "+" + LP_M_CostDetail.COLUMNNAME_amtll + ")");
	}
	 ...
	 * @return
	 */
	public static BigDecimal getQtyOnHandByASIAndSeqNo(Properties context, int productId, int costTypeId, int costElementId, int attributeSetInstanceId,
			int seqNo, String trxName) {
		List<Object> parameters = new ArrayList<Object>();
		StringBuilder whereClause = new StringBuilder();
		whereClause.append(LP_M_CostDetail.COLUMNNAME_M_Product_ID).append("=? AND ");
		parameters.add(productId);
		whereClause.append(LP_M_CostDetail.COLUMNNAME_M_CostType_ID).append("=? AND ");
		parameters.add(costTypeId);
		whereClause.append(LP_M_CostDetail.COLUMNNAME_M_CostElement_ID).append("=? AND ");
		parameters.add(costElementId);
		whereClause.append(LP_M_CostDetail.COLUMNNAME_M_AttributeSetInstance_ID).append("=? AND");
		parameters.add(attributeSetInstanceId);
		whereClause.append(LP_M_CostDetail.COLUMNNAME_SeqNo).append("<=?");
		parameters.add(seqNo);
		return new Query(context, Table_Name, whereClause.toString(), trxName).setParameters(parameters).sum(LP_M_CostDetail.COLUMNNAME_Qty);
	}
	...
	public static List<MCostDetail> getByTransaction(MTransaction transaction, int accountSchemaId, int costTypeId, int costElementId) {
		...
		return new Query(transaction.getCtx(), LP_M_CostDetail.Table_Name, whereClause.toString(), transaction.get_TrxName())
				.setParameters(accountSchemaId, costTypeId, costElementId, transaction.getM_Transaction_ID()).setOrderBy(MCostDetail.COLUMNNAME_SeqNo).list();
	}

	public static MCostDetail getCostDetail(MPPCostCollector cc, int M_CostElement_ID) {
		final String whereClause = MCostDetail.COLUMNNAME_PP_Cost_Collector_ID + "=?" + " AND " + MCostDetail.COLUMNNAME_M_CostElement_ID + "=?";
		MCostDetail cd = new Query(cc.getCtx(), MCostDetail.Table_Name, whereClause, cc.get_TrxName()).setClient_ID()
				.setParameters(new Object[] { cc.getPP_Cost_Collector_ID(), M_CostElement_ID }).firstOnly();
		return cd;
	}
	...
	public static BigDecimal getTotalCost(MCostDetail cost, MAcctSchema as) {
		return cost.getCostAmt().add(cost.getCostAdjustment()).add(cost.getCostAmtLL()).add(cost.getCostAdjustmentLL())
				.setScale(as.getCostingPrecision(), BigDecimal.ROUND_HALF_UP);
	}
	...
	public static List<MCostDetail> getByCollectorCost(MPPCostCollector costCollector) {
		...
		return new Query(costCollector.getCtx(), MCostDetail.Table_Name, whereClause.toString(), costCollector.get_TrxName()).setClient_ID()
				.setParameters(costCollector.getPP_Cost_Collector_ID()).list();
	}

	/**
	 * get the last entry for a Cost Detail based on the Material Transaction
	 * and Cost Dimension
	 * 
	 * @param model
	 * @param transaction
	 ...
	 * @return
	 */
	public static MCostDetail getLastTransaction(IDocumentLine model, MTransaction transaction, int C_AcctSchema_ID, int M_CostType_ID, int M_CostElement_ID,
		...
		if (model instanceof MLandedCostAllocation || model instanceof MMatchInv) {
			Timestamp dateacct = MUMTransaction.getDocumentLine(transaction).getDateAcct();
			MDocType dt = new MDocType(transaction.getCtx(), MUMTransaction.getDocumentLine(transaction).getC_DocType_ID(), transaction.get_TrxName());
		...
		if (LP_C_AcctSchema.COSTINGLEVEL_Organization.equals(costingLevel)) {
			whereClause.append(MCostDetail.COLUMNNAME_AD_Org_ID + "=? AND ");
			params.add(transaction.getAD_Org_ID());
		}

		if (LP_C_AcctSchema.COSTINGLEVEL_Warehouse.equals(costingLevel)) {
			whereClause.append(MCostDetail.COLUMNNAME_M_Warehouse_ID + "=? AND ");
			params.add(MUMTransaction.getM_Warehouse_ID(transaction));
		}
		...
		if (LP_C_AcctSchema.COSTINGLEVEL_BatchLot.equals(costingLevel)) {
		...
		/*
		 * List<MCostDetail> costs = new Query(transaction.getCtx(), Table_Name,
		 * whereClause.toString(), transaction.get_TrxName())
		 * .setParameters(params) .setOrderBy(orderBy.toString()) .list();
		 * 
		 * System.out.println(
		 * "---------------------- Transaccion -------------------------------------------------------"
		 * ); System.out.println(transaction.toString()); System.out.println(
		 * "------------------------------------------------------------------------------------------"
		 * ); for (MCostDetail cost : costs) {
		 * System.out.println(cost.toString()); } System.out.println(
		 * "---------------------- FIN BUSCANDO LA ULTIMA TRANSACCIONES ------------------------------"
		 * ); System.out.println("");
		 */

		return new Query(transaction.getCtx(), Table_Name, whereClause.toString(), transaction.get_TrxName()).setParameters(params)
				.setOrderBy(orderBy.toString()).first();
	}
	...
	public static MCostDetail getLastTransaction(MTransaction mtrx, int C_AcctSchema_ID, int M_CostType_ID, int M_CostElement_ID, String costingLevel) {
		...
		return new Query(mtrx.getCtx(), Table_Name, whereClause.toString(), mtrx.get_TrxName()).setParameters(params)
				.setOrderBy(MCostDetail.COLUMNNAME_SeqNo + " DESC").first();
	}
	...
	public static MCostDetail getByTransaction(IDocumentLine model, MTransaction mtrx, int C_AcctSchema_ID, int M_CostType_ID, int M_CostElement_ID) {
		...
		params.add(MUMTransaction.getM_Warehouse_ID(mtrx));
		whereClause.append(MCostDetail.COLUMNNAME_C_AcctSchema_ID).append("=? AND ");
		params.add(C_AcctSchema_ID);
		whereClause.append(MCostDetail.COLUMNNAME_M_Product_ID).append("=? AND ");
		params.add(mtrx.getM_Product_ID());
		...
		return new Query(mtrx.getCtx(), LP_M_CostDetail.Table_Name, whereClause.toString(), mtrx.get_TrxName()).setParameters(params)
				.setOrderBy(MCostDetail.COLUMNNAME_SeqNo + " DESC").first();
	}
	 ...
	 * @param costTypeId
	 *            Cost type
	 * @param isExcludeLandedCost
	 *            exclude cost detail for landed cost
	 * @return list MCostDetail
	 */
	/**
	 * Libero to Lybertia migration unnesesary method
	 */
	/*
	 * public static List<MCostDetail> getByDocLine(DocLine docLine, int
	 * accountSchemaId, int costTypeId, boolean isExcludeLandedCost) {
	 * StringBuilder whereClause = new StringBuilder();
	 * whereClause.append(MCostDetail
	 * .COLUMNNAME_AD_Client_ID).append("=? AND ");
	 * whereClause.append(MCostDetail
	 * .COLUMNNAME_C_AcctSchema_ID).append("=? AND ");
	 * whereClause.append(MCostDetail
	 * .COLUMNNAME_M_Product_ID).append("=? AND ");
	 * whereClause.append(MCostDetail
	 * .COLUMNNAME_M_CostType_ID).append("=? AND "); if (isExcludeLandedCost)
	 * whereClause
	 * .append(MCostDetail.COLUMNNAME_C_LandedCostAllocation_ID).append
	 * (" IS NULL AND ");
	 * 
	 * whereClause.append(docLine.getTableName()).append("_ID=?"); return new
	 * Query(docLine.getCtx(), LP_M_CostDetail.Table_Name,
	 * whereClause.toString(),
	 * docLine.getTrxName()).setParameters(docLine.getAD_Client_ID(),
	 * accountSchemaId, docLine.getM_Product_ID(), //
	 * docLine.getM_AttributeSetInstance_ID(), costTypeId,
	 * docLine.get_ID()).list(); }
	 */
	...
	public static List<MCostDetail> getAfterDate(MCostDetail cd, String costingLevel) {
		...
		return new Query(cd.getCtx(), Table_Name, whereClause.toString(), cd.get_TrxName()).setClient_ID().setParameters(params)
				.setOrderBy(MCostDetail.COLUMNNAME_SeqNo + " ASC").list();
	}
	 ...
	 *            order
	 * @param M_CostElement_ID
	 *            optional cost element for Freight
	 * @param Amt
	 *            amt total amount
	 * @param Qty
	 *            qty
	 ...
	 */
	public static boolean createOrder(MAcctSchema as, int AD_Org_ID, int M_Product_ID, int M_AttributeSetInstance_ID, int C_OrderLine_ID, int M_CostElement_ID,
			BigDecimal Amt, BigDecimal Qty, String Description, String trxName) {
		// Delete Unprocessed zero Differences
		String sql = "DELETE M_CostDetail " + "WHERE Processed='N' AND COALESCE(DeltaAmt,0)=0 AND COALESCE(DeltaQty,0)=0" + " AND C_OrderLine_ID="
				+ C_OrderLine_ID + " AND C_AcctSchema_ID =" + as.getC_AcctSchema_ID() + " AND M_AttributeSetInstance_ID=" + M_AttributeSetInstance_ID;
		int no = DB.executeUpdate(sql, trxName);
		if (no != 0)
			s_log.config("Deleted #" + no);
		MCostDetail cd = get(as.getCtx(), "C_OrderLine_ID=?", C_OrderLine_ID, M_AttributeSetInstance_ID, as.getC_AcctSchema_ID(), trxName);
		//
		if (cd == null) // createNew
		{
			...
			// MZ Goodwill
			// set deltaAmt=Amt, deltaQty=qty, and set Cost Detail for Amt and
			// Qty
	...
	/**
	 * Create New Invoice Cost Detail for AP Invoices. Called from Doc_Invoice -
	 ...
	 * @return true if created
	 */
	public static boolean createInvoice(MAcctSchema as, int AD_Org_ID, int M_Product_ID, int M_AttributeSetInstance_ID, int C_InvoiceLine_ID,
			int M_CostElement_ID, BigDecimal Amt, BigDecimal Qty, String Description, String trxName) {
		// Delete Unprocessed zero Differences
		String sql = "DELETE M_CostDetail " + "WHERE Processed='N' AND COALESCE(DeltaAmt,0)=0 AND COALESCE(DeltaQty,0)=0" + " AND C_InvoiceLine_ID="
				+ C_InvoiceLine_ID + " AND C_AcctSchema_ID =" + as.getC_AcctSchema_ID() + " AND M_AttributeSetInstance_ID=" + M_AttributeSetInstance_ID;
		int no = DB.executeUpdate(sql, trxName);
		if (no != 0)
			s_log.config("Deleted #" + no);
		MCostDetail cd = get(as.getCtx(), "C_InvoiceLine_ID=?", C_InvoiceLine_ID, M_AttributeSetInstance_ID, as.getC_AcctSchema_ID(), trxName);
		//
		if (cd == null) // createNew
		{
			cd = new MCostDetail(as, AD_Org_ID, M_Product_ID, M_AttributeSetInstance_ID, M_CostElement_ID, Amt, Qty, Description, trxName);
	...
	/**
	 * Create New Shipment Cost Detail for SO Shipments. Called from Doc_MInOut
	 * - for SO Shipments
	 *
	 * @param as
	 *            accounting schema
	 ...
	 */
	public static boolean createShipment(MAcctSchema as, int AD_Org_ID, int M_Product_ID, int M_AttributeSetInstance_ID, int M_InOutLine_ID,
			int M_CostElement_ID, BigDecimal Amt, BigDecimal Qty, String Description, boolean IsSOTrx, String trxName) {
		// Delete Unprocessed zero Differences
		String sql = "DELETE M_CostDetail " + "WHERE Processed='N' AND COALESCE(DeltaAmt,0)=0 AND COALESCE(DeltaQty,0)=0" + " AND M_InOutLine_ID="
				+ M_InOutLine_ID + " AND C_AcctSchema_ID =" + as.getC_AcctSchema_ID() + " AND M_AttributeSetInstance_ID=" + M_AttributeSetInstance_ID;
		int no = DB.executeUpdate(sql, trxName);
		if (no != 0)
			s_log.config("Deleted #" + no);
		MCostDetail cd = get(as.getCtx(), "M_InOutLine_ID=?", M_InOutLine_ID, M_AttributeSetInstance_ID, as.getC_AcctSchema_ID(), trxName);
		//
		if (cd == null) // createNew
		{
			cd = new MCostDetail(as, AD_Org_ID, M_Product_ID, M_AttributeSetInstance_ID, M_CostElement_ID, Amt, Qty, Description, trxName);
	...
	/**
	 * Create New Order Cost Detail for Physical Inventory. Called from
	 * Doc_Inventory
	 *
	 * @param as
	 *            accounting schema
	 ...
	 * @return true if no error
	 */
	public static boolean createInventory(MAcctSchema as, int AD_Org_ID, int M_Product_ID, int M_AttributeSetInstance_ID, int M_InventoryLine_ID,
			int M_CostElement_ID, BigDecimal Amt, BigDecimal Qty, String Description, String trxName) {
		// Delete Unprocessed zero Differences
		String sql = "DELETE M_CostDetail " + "WHERE Processed='N' AND COALESCE(DeltaAmt,0)=0 AND COALESCE(DeltaQty,0)=0" + " AND M_InventoryLine_ID="
				+ M_InventoryLine_ID + " AND C_AcctSchema_ID =" + as.getC_AcctSchema_ID() + " AND M_AttributeSetInstance_ID=" + M_AttributeSetInstance_ID;
		int no = DB.executeUpdate(sql, trxName);
		if (no != 0)
			s_log.config("Deleted #" + no);
		MCostDetail cd = get(as.getCtx(), "M_InventoryLine_ID=?", M_InventoryLine_ID, M_AttributeSetInstance_ID, as.getC_AcctSchema_ID(), trxName);
		//
		if (cd == null) // createNew
		{
			cd = new MCostDetail(as, AD_Org_ID, M_Product_ID, M_AttributeSetInstance_ID, M_CostElement_ID, Amt, Qty, Description, trxName);
	 ...
	 * @return true if no error
	 */
	public static boolean createMovement(MAcctSchema as, int AD_Org_ID, int M_Product_ID, int M_AttributeSetInstance_ID, int M_MovementLine_ID,
			int M_CostElement_ID, BigDecimal Amt, BigDecimal Qty, boolean from, String Description, String trxName) {
		// Delete Unprocessed zero Differences
		String sql = "DELETE M_CostDetail " + "WHERE Processed='N' AND COALESCE(DeltaAmt,0)=0 AND COALESCE(DeltaQty,0)=0" + " AND M_MovementLine_ID="
				+ M_MovementLine_ID + " AND IsSOTrx=" + (from ? "'Y'" : "'N'") + " AND C_AcctSchema_ID =" + as.getC_AcctSchema_ID()
				+ " AND M_AttributeSetInstance_ID=" + M_AttributeSetInstance_ID;
		int no = DB.executeUpdate(sql, trxName);
		if (no != 0)
			s_log.config("Deleted #" + no);
		MCostDetail cd = get(as.getCtx(), "M_MovementLine_ID=? AND IsSOTrx=" + (from ? "'Y'" : "'N'"), M_MovementLine_ID, M_AttributeSetInstance_ID,
				as.getC_AcctSchema_ID(), trxName);
		//
		if (cd == null) // createNew
		{
			cd = new MCostDetail(as, AD_Org_ID, M_Product_ID, M_AttributeSetInstance_ID, M_CostElement_ID, Amt, Qty, Description, trxName);
	 ...
	 * @return true if no error
	 */
	public static boolean createProduction(MAcctSchema as, int AD_Org_ID, int M_Product_ID, int M_AttributeSetInstance_ID, int M_ProductionLine_ID,
			int M_CostElement_ID, BigDecimal Amt, BigDecimal Qty, String Description, String trxName) {
		// Delete Unprocessed zero Differences
		String sql = "DELETE M_CostDetail " + "WHERE Processed='N' AND COALESCE(DeltaAmt,0)=0 AND COALESCE(DeltaQty,0)=0" + " AND M_ProductionLine_ID="
				+ M_ProductionLine_ID + " AND C_AcctSchema_ID =" + as.getC_AcctSchema_ID() + " AND M_AttributeSetInstance_ID=" + M_AttributeSetInstance_ID;
		int no = DB.executeUpdate(sql, trxName);
		if (no != 0)
			s_log.config("Deleted #" + no);
		MCostDetail cd = get(as.getCtx(), "M_ProductionLine_ID=?", M_ProductionLine_ID, M_AttributeSetInstance_ID, as.getC_AcctSchema_ID(), trxName);
		//
		if (cd == null) // createNew
		{
			cd = new MCostDetail(as, AD_Org_ID, M_Product_ID, M_AttributeSetInstance_ID, M_CostElement_ID, Amt, Qty, Description, trxName);
	...
	public static MCostDetail get(Properties ctx, String whereClause, int ID, int M_AttributeSetInstance_ID, int C_AcctSchema_ID, String trxName) {
		final String localWhereClause = whereClause + " AND M_AttributeSetInstance_ID=?" + " AND C_AcctSchema_ID=?";
		MCostDetail retValue = new Query(ctx, LP_M_CostDetail.Table_Name, localWhereClause, trxName).setParameters(ID, M_AttributeSetInstance_ID,
	...
	public static boolean processProduct(MProduct product, String trxName) {
		final String whereClause = LP_M_CostDetail.COLUMNNAME_M_Product_ID + "=?" + " AND " + LP_M_CostDetail.COLUMNNAME_Processed + "=?";
		int counterOK = 0;
		int counterError = 0;
		List<MCostDetail> list = new Query(product.getCtx(), LP_M_CostDetail.Table_Name, whereClause, trxName).setParameters(product.getM_Product_ID(), false)
				.setOrderBy("C_AcctSchema_ID, M_CostElement_ID, AD_Org_ID, M_Warehouse_ID, M_AttributeSetInstance_ID, Created").list();
	 ...
	 * @param M_CostType_ID
	 */
	public MCostDetail(MAcctSchema as, int AD_Org_ID, int M_Product_ID, int M_AttributeSetInstance_ID, int M_CostElement_ID, BigDecimal Amt, BigDecimal Qty,
	 ...
	 * @param trxName
	 */
	public MCostDetail(MTransaction transaction, int acctSchemaId, int costTypeId, int costElementId, BigDecimal amt, BigDecimal amtLL, BigDecimal qty,
			String trxName) {
		this(transaction.getCtx(), 0, trxName);
		setAD_Client_ID(transaction.getAD_Client_ID());
		setAD_Org_ID(transaction.getAD_Org_ID());
		setM_Warehouse_ID(MUMTransaction.getM_Warehouse_ID(transaction));
	...
	public String toString() {
		 ...
		 * sb.append(",C_LandedCostAllocation_ID="
		 * ).append(getC_LandedCostAllocation_ID());
		 */
	...
	public synchronized boolean process() {
		...
		if (getM_CostElement_ID() == 0) {
			for (MCostElement costElement : MUMCostElement.getCostElement(getCtx(), get_TrxName())) {
			...
			MCostElement ce = new MCostElement(getCtx(), getM_CostElement_ID(), null);
	...
	protected boolean afterDelete(boolean success) {
		if (success) {
			...
			if (getM_CostElement_ID() == 0) {
				...
				MCostElement ce = MUMCostElement.get(getCtx(), getM_CostElement_ID());
				ok = process(as, product, ce, organizationId, warehouseId, attributeSetInstanceId);
			}
	...
	private void setDateAcct(boolean force) {
		...
		if (getC_InvoiceLine_ID() > 0) {
			...
			sql = "SELECT i.MovementDate FROM M_InventoryLine il" + " INNER JOIN M_Inventory i ON (i.M_Inventory_ID = il.M_Inventory_ID)"
					+ " WHERE il.M_InventoryLine_ID=?";
			param1 = getM_InventoryLine_ID();
		} else if (getM_MovementLine_ID() > 0) {
			sql = "SELECT i.MovementDate FROM M_MovementLine il" + " INNER JOIN M_Movement i ON (i.M_Movement_ID = il.M_Movement_ID)"
					+ " WHERE il.M_MovementLine_ID=?";
			param1 = getM_MovementLine_ID();
		} /*
		 * else if (getC_LandedCostAllocation_ID() > 0) { sql =
		 * "SELECT i.DateAcct FROM C_Invoice i" +
		 * " INNER JOIN C_InvoiceLine il ON (i.C_Invoice_ID=il.C_Invoice_ID)" +
		 * " INNER JOIN C_LandedCostAllocation la ON (il.C_InvoiceLine_ID=la.C_InvoiceLine_ID)"
		 * + " WHERE la.C_LandedCostAllocation_ID=?"; param1 =
		 * getC_LandedCostAllocation_ID(); }
		 */
		//
		dateAcct = MUDB.getSQLValueTSEx(get_TrxName(), sql, param1);
	...
	private void rePosted() {
		if (getC_InvoiceLine_ID() > 0) {
			...
			if (id > 0) {
				DB.executeUpdate("UPDATE M_MatchInv SET Posted='N', Processing='N', ProcessedOn=null WHERE M_MatchInv_ID=" + id + " AND Processed='Y'",
						get_TrxName());
				MFactAcct.delete(MMatchInv.Table_ID, id, get_TrxName());
			}
		} else if (getM_InOutLine_ID() > 0) {
			int id = DB.getSQLValue(get_TrxName(), "SELECT M_InOut_ID FROM M_InOutLine WHERE M_InOutLine_ID=? ", getM_InOutLine_ID());
			if (id > 0) {
				DB.executeUpdate("UPDATE M_InOut SET Posted='N', Processing='N', ProcessedOn=null WHERE M_InOut_ID=" + id + " AND Processed='Y'", get_TrxName());
				MFactAcct.delete(MInOut.Table_ID, id, get_TrxName());
			}
		} else if (getC_OrderLine_ID() > 0) {
			int id = DB.getSQLValue(get_TrxName(), "SELECT M_MatchPO_ID FROM M_MatchPO WHERE C_OrderLine_ID=" + getC_OrderLine_ID());
			if (id > 0) {
				DB.executeUpdate("UPDATE M_MatchPO SET Posted='N', Processing='N', ProcessedOn=null WHERE M_MatchPO_ID=" + id + " AND Processed='Y'",
						get_TrxName());
				MFactAcct.delete(MMatchPO.Table_ID, id, get_TrxName());
			}
		} else if (getM_InventoryLine_ID() > 0) {
			...
			if (id > 0) {
				DB.executeUpdate("UPDATE M_Inventory SET Posted='N', Processing='N', ProcessedOn=null WHERE M_Inventory_ID=" + id + " AND Processed='Y'",
						get_TrxName());
				MFactAcct.delete(MInventory.Table_ID, id, get_TrxName());
			}
		} else if (getM_MovementLine_ID() > 0) {
			int id = DB.getSQLValue(get_TrxName(), "SELECT M_Movement_ID FROM M_MovementLine WHERE M_MovementLine_ID=?", getM_MovementLine_ID());
			if (id > 0) {
				DB.executeUpdate("UPDATE M_Movement SET Posted='N', Processing='N', ProcessedOn=null WHERE M_Movement_ID=" + id + " AND Processed='Y'",
						get_TrxName());
				MFactAcct.delete(MMovement.Table_ID, id, get_TrxName());
			}
		}/*
		 * else if (getC_LandedCostAllocation_ID() > 0) { // Is necessary the
		 * logic when exist a landen cost }
		 */
	}
	...
	public int getM_Warehouse_ID() {
		final String whereClause = "SELECT l.M_Warehouse_ID FROM M_CostDetail cd " + "INNER JOIN  M_Transaction t ON (cd.M_Transaction_ID=t.M_Transaction_ID) "
				+ "INNER JOIN M_Locator l ON (t.M_Locator_ID=l.M_Locator_ID) WHERE cd.M_CostDetail_ID=? ";
		return DB.getSQLValue(this.get_TrxName(), whereClause, getID());
	}
	...
	public static BigDecimal getByDocLineLandedCost(LP_C_LandedCostAllocation landedCostAllocation, int acctSchemaId, int costTypeId) {
		StringBuffer whereClause = new StringBuffer();
		whereClause.append(COLUMNNAME_AD_Client_ID).append("=? AND ").append(COLUMNNAME_C_AcctSchema_ID).append("=? AND ").append(COLUMNNAME_M_Product_ID)
				.append("=? AND ").append(COLUMNNAME_M_AttributeSetInstance_ID).append("=? AND ").append(COLUMNNAME_M_CostType_ID).append("=? AND ")
				.append(COLUMNNAME_C_InvoiceLine_ID).append("=? AND ").append(COLUMNNAME_M_InOutLine_ID).append("=? ");
		return new Query(landedCostAllocation.getCtx(), Table_Name, whereClause.toString(), landedCostAllocation.get_TrxName()).setParameters(
				landedCostAllocation.getAD_Client_ID(), acctSchemaId, landedCostAllocation.getM_Product_ID(), costTypeId,
				landedCostAllocation.getC_InvoiceLine_ID(), landedCostAllocation.getM_InOutLine_ID())

		.aggregate(COLUMNNAME_costamt + " + " + COLUMNNAME_costadjustment + " + " + COLUMNNAME_costamtll + " + " + COLUMNNAME_costadjustmentll,
				Query.AGGREGATE_SUM);
	}
	...
	public static BigDecimal getByDocLineMatchInv(MInvoiceLine invoiceLine, MInOutLine inOutLine, int acctSchemaId, int costTypeId) {
		...
		whereClause.append(COLUMNNAME_AD_Client_ID).append("=? AND ").append(COLUMNNAME_C_AcctSchema_ID).append("=? AND ").append(COLUMNNAME_M_Product_ID)
				.append("=? AND ").append(COLUMNNAME_M_AttributeSetInstance_ID + "=? AND ").append(COLUMNNAME_M_CostType_ID).append("=? AND ")
				.append(COLUMNNAME_C_InvoiceLine_ID).append("=? AND ").append(COLUMNNAME_M_InOutLine_ID).append("=?");

		return new Query(invoiceLine.getCtx(), LP_M_CostDetail.Table_Name, whereClause.toString(), invoiceLine.get_TrxName()).setParameters(
				invoiceLine.getAD_Client_ID(), acctSchemaId, invoiceLine.getM_Product_ID(), costTypeId, invoiceLine.getC_InvoiceLine_ID(),
				inOutLine.getM_InOutLine_ID()).aggregate(COLUMNNAME_costadjustment + " + " + COLUMNNAME_costadjustmentll, Query.AGGREGATE_SUM);
	}
	...
	public MAcctSchema getC_AcctSchema() {
		// TODO Auto-generated method stub
		return new MAcctSchema(getCtx(), getC_AcctSchema_ID(), get_TrxName());
	}
} // MCostDetail
