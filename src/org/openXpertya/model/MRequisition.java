 ...
 * @author red1 <li>FR [ 2214883 ] Remove SQL code and Replace for Query
 * @author Teo Sarca, www.arhipac.ro <li>FR [ 2744682 ] Requisition: improve
 *         error reporting
 */
public class MRequisition extends LP_M_Requisition implements DocAction {
	/**
	 * 
	 */
	...
	/**
	 * PO.setTrxName - set given trxName to an array of POs As suggested by teo
	 * in [ 1854603 ]
	 */
	...
	public MRequisitionLine[] getLines() {
		...
		final String whereClause = MUColumnNames.COLUMNNAME_M_Requisition_ID + "=?";
		List<MRequisitionLine> list = new Query(getCtx(), MRequisitionLine.Table_Name, whereClause, get_TrxName()).setParameters(getID())
				.setOrderBy(MUColumnNames.COLUMNNAME_Line).list();
		// red1 - end -
	...
	private void setDefiniteDocumentNo() {
		...
		/*
		 * if (dt.isOverwriteDateOnComplete()) { setDateDoc(new Timestamp
		 * (System.currentTimeMillis())); } if (dt.isOverwriteSeqOnComplete()) {
		 * String value = DB.getDocumentNo(getC_DocType_ID(), get_TrxName(),
		 * true, this); DB.getDocumentNo if (value != null)
		 * setDocumentNo(value); }
		 */
	}
