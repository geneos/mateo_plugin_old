...
public class VOrderReceiptIssue extends OrderReceiptIssue implements FormPanel,
	...
	private void fillPicks() throws Exception {
		...
		MLookup orderL = MLookupFactory.get(ctx, m_WindowNo, M_Column.getColumnID(null, MPPOrder.COLUMNNAME_PP_Order_ID, MPPOrder.Table_Name),
				DisplayType.Search,
				language, "PP_Order_ID", 0, false, "PP_Order.DocStatus = '"
						+ MPPOrder.ACTION_Complete + "'");

		orderField = new VLookup("PP_Order_ID", false, false, true, orderL);
		orderField.setBackground(CompierePLAF.getInfoBackground());
		orderField.addVetoableChangeListener(this);

		MLookup resourceL = MLookupFactory.get(ctx, m_WindowNo, 0, M_Column.getColumnID(null, MPPOrder.COLUMNNAME_S_Resource_ID, MPPOrder.Table_Name),
				DisplayType.TableDir);
		resourceField = new VLookup("S_Resource_ID", false, false, false,
				resourceL);

		MLookup warehouseL = MLookupFactory.get(ctx, m_WindowNo, 0, M_Column.getColumnID(null, MPPOrder.COLUMNNAME_M_Warehouse_ID, MPPOrder.Table_Name),
				DisplayType.TableDir);
		warehouseField = new VLookup("M_Warehouse_ID", false, false, false,
				warehouseL);

		MLookup productL = MLookupFactory
				.get(ctx, m_WindowNo, 0, M_Column.getColumnID(null, MPPOrder.COLUMNNAME_M_Product_ID, MPPOrder.Table_Name),
						DisplayType.TableDir);
		productField = new VLookup("M_Product_ID", false, false, false,
				productL);

		MLookup uomL = MLookupFactory.get(ctx, m_WindowNo, 0,M_Column.getColumnID(null, MPPOrder.COLUMNNAME_C_UOM_ID, MPPOrder.Table_Name),
				DisplayType.TableDir);
		uomField = new VLookup("C_UOM_ID", false, false, false, uomL);

		MLookup uomorderL = MLookupFactory.get(ctx, m_WindowNo, 0,M_Column.getColumnID(null, MPPOrder.COLUMNNAME_C_UOM_ID, MPPOrder.Table_Name),
				DisplayType.TableDir);
        ...
        MFieldVO vo =  MFieldVO.createStdField(ctx , m_WindowNo , MUMTab.getTab_ID(m_Window, "Order") , m_Window, false, false, false);
        // M_AttributeSetInstance_ID
        vo.AD_Column_ID = M_Column.getColumnID(null, MPPOrder.COLUMNNAME_M_AttributeSetInstance_ID, MPPOrder.Table_Name);            
