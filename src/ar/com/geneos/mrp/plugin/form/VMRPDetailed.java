...
import ar.com.geneos.mrp.plugin.model.LP_PP_Product_Planning;
 ...
 */
public class VMRPDetailed extends MRPDetailed implements FormPanel, ActionListener, VetoableChangeListener, ChangeListener, ListSelectionListener,
		...
		public void run() {
			...
			if (getM_Product_ID() > 0) {
				...
				for (int row = 0; row < p_table.getRowCount(); row++) {
					...
					if (LP_PP_MRP.TYPEMRP_Demand.equals(TypeMRP) || (LP_PP_MRP.ORDERTYPE_Forecast.equals(OrderType) // TODO:
																													// arhipac:
																													// teo_sarca:
																													// is
																													// this
					...
					if (LP_PP_MRP.TYPEMRP_Supply.equals(TypeMRP)) {
	...
	/** Window Width */
	static final int INFO_WIDTH = 1100;
	...
	//
	private CLabel lDateFrom = new CLabel(Msg.translate(getCtx(), MUColumnNames.COLUMNNAME_DateFrom));

	// DueStart Field
	private VDate fDateFrom = new VDate(MUColumnNames.COLUMNNAME_DateFrom, false, false, true, DisplayType.Date, Msg.translate(getCtx(),
			MUColumnNames.COLUMNNAME_DateFrom)) {
		private static final long serialVersionUID = 1L;
	...
	private CLabel lDateTo = new CLabel(Msg.translate(getCtx(), MUColumnNames.COLUMNNAME_DateTo));

	// DueEnd Field
	private VDate fDateTo = new VDate(MUColumnNames.COLUMNNAME_DateTo, false, false, true, DisplayType.Date, Msg.translate(getCtx(),
			MUColumnNames.COLUMNNAME_DateTo)) {
		private static final long serialVersionUID = 1L;
	...
	/*private VCheckBox fMaster = new VCheckBox(MPPProductPlanning.COLUMNNAME_IsMPS, false, false, true, Msg.translate(getCtx(),
			MPPProductPlanning.COLUMNNAME_IsMPS), "", false);*/
	private VCheckBox fMRPReq = new VCheckBox(MPPProductPlanning.COLUMNNAME_IsRequiredMRP, false, false, true, Msg.translate(getCtx(),
			MPPProductPlanning.COLUMNNAME_IsRequiredMRP), "", false);
	private VCheckBox fCreatePlan = new VCheckBox(MPPProductPlanning.COLUMNNAME_IsCreatePlan, false, false, true, Msg.translate(getCtx(),
			MPPProductPlanning.COLUMNNAME_IsCreatePlan), "", false);
	...
	private void fillHead() {
		...
		fType.setText(MRefList.getListName(getCtx(), LP_PP_Product_Planning.ORDER_POLICY_AD_Reference_ID, pp.getOrder_Policy()));
...
*/
	...
	private String getSQLWhere() {
		...
		if (fProduct_ID.getValue() != null) {
			...
			sql.append(" AND ((" + getTableName() + ".OrderType IN ('SOO','MOP','POO','POR','STK','DOO','FCT')) OR (" + getTableName() + ".OrderType='FCT' AND "
					+ getTableName() + ".DatePromised >= SYSDATE))");
			fillHead();
			setMRP();
		}
/*
...
*/
	...
	public void init(int WindowNo, FormFrame frame) {
		m_WindowNo = WindowNo;
		m_frame = frame;
		Env.setContext(getCtx(), m_WindowNo, MUColumnNames.COLUMNNAME_IsSOTrx, "N");
	...
	private void selectAttributeSetInstance() {
		...
		//String title = product.get_Translation(MUColumnNames.COLUMNNAME_Name) + " - " + wh.get_Translation(MUColumnNames.COLUMNNAME_Name);
		String title = product.getName() + " - " + wh.getName();

		PAttributeInstance pai = new PAttributeInstance(m_frame, title, m_warehouse_id, 0, m_product_id, 0, 0);
	...
	private void setMRP() {
		...
		try {
			/*
			StringBuffer sql = new StringBuffer("SELECT ").append("BOMQtyOnHandASI(M_Product_ID,?,?,?) as qtyonhand, ")
					.append("BOMQtyReservedASI(M_Product_ID,?,?,?) as qtyreserved, ").append("BOMQtyAvailableASI(M_Product_ID,?,?,?) as qtyavailable, ")
					.append("BOMQtyOrderedASI(M_Product_ID,?,?,?) as qtyordered").append(" FROM M_Product WHERE M_Product_ID=?");*/
			StringBuffer sql = new StringBuffer("SELECT ").append("BOMQtyOnHand(M_Product_ID,?,?) as qtyonhand, ")
					.append("BOMQtyReserved(M_Product_ID,?,?) as qtyreserved, ").append("BOMQtyAvailable(M_Product_ID,?,?) as qtyavailable, ")
					.append("BOMQtyOrdered(M_Product_ID,?,?) as qtyordered").append(" FROM M_Product WHERE M_Product_ID=?");
			pstmt = DB.prepareStatement(sql.toString(), null);
			DB.setParameters(pstmt, new Object[] { /*getM_AttributeSetInstance_ID(),*/ getM_Warehouse_ID(), 0/*, getM_AttributeSetInstance_ID()*/, getM_Warehouse_ID(),
					0/*, getM_AttributeSetInstance_ID()*/, getM_Warehouse_ID(), 0/*, getM_AttributeSetInstance_ID()*/, getM_Warehouse_ID(), 0, getM_Product_ID() });
		...
		KeyNamePair kum = new KeyNamePair(um.getC_UOM_ID(), um.get_Translation(MUColumnNames.COLUMNNAME_Name));
		...
		if (getM_Warehouse_ID() > 0) {
			...
			replenishLevelMin = MUDB.getSQLValueBD(null, sql, AD_Client_ID, M_Product_ID, M_Warehouse_ID);
		}
	...
	private void statInit() throws Exception {
		...
		MLookup resourceL = MLookupFactory.get(getCtx(), p_WindowNo, MUMColumn.getColumn_ID(MResource.Table_Name, MUColumnNames.COLUMNNAME_S_Resource_ID),
				DisplayType.TableDir, language, MUColumnNames.COLUMNNAME_S_Resource_ID, 0, false, MResource.Table_Name + "."
						+ MUColumnNames.COLUMNNAME_ManufacturingResourceType + "= '" + MResource.MANUFACTURINGRESOURCETYPE_Plant + "'");
		...
		// Planner Lookup
		fPlanner_ID = new VLookup(MPPMRP.COLUMNNAME_Planner_ID, false, false, true, MLookupFactory.get(getCtx(), p_WindowNo, 0,
				MUMColumn.getColumn_ID(MPPProductPlanning.Table_Name, MPPMRP.COLUMNNAME_Planner_ID), DisplayType.Table)) {
			private static final long serialVersionUID = 1L;
		...
		// Wahrehouse Lookup
		fWarehouse_ID = new VLookup(MPPMRP.COLUMNNAME_M_Warehouse_ID, true, false, true, MLookupFactory.get(getCtx(), p_WindowNo, 0,
				MUMColumn.getColumn_ID(MWarehouse.Table_Name, MPPMRP.COLUMNNAME_M_Warehouse_ID), DisplayType.TableDir)) {
			private static final long serialVersionUID = 1L;
		...
		lUOM.setText(Msg.translate(getCtx(), MUColumnNames.COLUMNNAME_C_UOM_ID));
		...
		lOnhand.setText(Msg.translate(getCtx(), MUColumnNames.COLUMNNAME_QtyOnHand));
		...
		lReserved.setText(Msg.translate(getCtx(), MUColumnNames.COLUMNNAME_QtyReserved));
		...
		// Product Lookup
		fProduct_ID = new VLookup(MPPMRP.COLUMNNAME_M_Product_ID, true, false, true, MLookupFactory.get(getCtx(), p_WindowNo, 0,
				MUMColumn.getColumn_ID(MProduct.Table_Name, MPPMRP.COLUMNNAME_M_Product_ID), DisplayType.Search)) {
			private static final long serialVersionUID = 1L;
		...
		fDateFrom.setToolTipText(Msg.translate(getCtx(), MUColumnNames.COLUMNNAME_DateFrom));
		lDateTo.setLabelFor(fDateTo);
		// fDateTo.setBackground(AdempierePLAF.getInfoBackground());
		fDateTo.setToolTipText(Msg.translate(getCtx(), MUColumnNames.COLUMNNAME_DateTo));
		fSupplyType = new VLookup(MPPMRP.COLUMNNAME_TypeMRP, false, false, true, MLookupFactory.get(getCtx(), p_WindowNo, 0,
				MUMColumn.getColumn_ID(MPPMRP.Table_Name, MPPMRP.COLUMNNAME_TypeMRP), DisplayType.List));
