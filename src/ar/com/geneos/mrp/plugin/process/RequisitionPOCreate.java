...
public class RequisitionPOCreate extends SvrProcess {
	...
	private void process(MRequisitionLine rLine) throws Exception {
		if (rLine.getM_Product_ID() == 0 && rLine.getC_Charge_ID() == 0) {
			log.warning("Ignored Line" + rLine.getLine() + " " + rLine.getDescription() + " - " + rLine.getLineNetAmt());
			return;
		}
		...
		if (m_orderLine == null || rLine.getM_Product_ID() != m_M_Product_ID || rLine.getM_AttributeSetInstance_ID() != m_M_AttributeSetInstance_ID
				|| rLine.getC_Charge_ID() != 0 // single line per charge
				|| m_order == null || m_order.getDatePromised().compareTo(rLine.getDateRequired()) != 0) {
			newLine(rLine);
	...
	private void newOrder(MRequisitionLine rLine, int C_BPartner_ID) throws Exception {
		...
		m_order = findOrder(C_BPartner_ID, DateRequired, M_PriceList_ID);
		if (m_order == null) {
	...
	private void closeOrder() throws Exception {
		...
		if (m_order != null) {
			...
			if (m_order.getLines(true).length == 0 ){
				addLog(0, null, m_order.getGrandTotal(), m_order.getDocumentNo() + " - se omite porque no tiene lineas.");
				m_order.delete(true);
			}
			else
