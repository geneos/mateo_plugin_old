...
public class MRP extends SvrProcess {
	...
	protected void prepare() {
		for (ProcessInfoParameter para : getParameter()) {
			...
			if (para.getParameter() == null)
				...
				p_IsSynchronize = "Y".equals(para.getParameter());
			else if (name.equals("IsMPS"))
				p_IsMPS = "Y".equals(para.getParameter());
			else if (name.equals("IsRequiredMRP"))
				p_IsMRP = "Y".equals(para.getParameter());
			else if (name.equals("IsRequiredDRP"))
				p_IsDRP = "Y".equals(para.getParameter());
			else if (name.equals("IsBOM"))
				p_IsMfg = "Y".equals(para.getParameter());
			else if (name.equals("IsPurchased"))
				p_IsPur = "Y".equals(para.getParameter());
			else if (name.equals("LowLevel"))
				p_IsLowLevel = "Y".equals(para.getParameter());
	...
	public Integer getPlanner_ID() {
		/*
		 * if (this.p_Planner_ID == null) { this.p_Planner_ID =
	...
	protected String doIt() throws Exception {
		...
		for (MResource plant : plants) {
			...
			for (MOrg org : orgList) {
				 ...
				 * MDocType.DOCBASETYPE_DistributionOrder, org.getAD_Org_ID(),
				 * AD_User_ID, get_TrxName());
				 */
				...
				for (MWarehouse w : ws) {
					...
					// if(plant.getM_Warehouse_ID() == w.getM_Warehouse_ID() &&
					// isRequiredDRP())
					// continue;
	...
	protected String getDeleteSQLWhere(String tableName, String where, Integer AD_Client_ID, Integer AD_Org_ID, Integer M_Warehouse_ID, Integer S_Resource_ID,
			Integer M_Product_ID, String typeMRP) {
	...
	/**************************************************************************
	 * Delete old record in MRP table to calculate again MRP and Document with
	 * Draft status
	 * 
	 * @param AD_Client_ID
	 *            Client_ID
	...
	protected void deleteMRP(int AD_Client_ID, int AD_Org_ID, int S_Resource_ID, int M_Warehouse_ID, int M_Product_ID, String trxName) throws SQLException {
		...
		sql.append(
				getDeleteSQLWhere("mrp", "mrp." + MPPMRP.COLUMNNAME_PP_MRP_ID + " = Record_ID AND AD_Table_ID=" + MPPMRP.Table_ID, AD_Client_ID, AD_Org_ID,
						null, null, M_Product_ID, null)).append(
				" AND (Reference LIKE '%M_Warehouse_ID->" + M_Warehouse_ID + "%') OR  (Reference LIKE '%M_Product_ID->" + M_Product_ID + "%' )");
		ArrayList<Object> myParameters = new ArrayList(parameters);
		DB.executeUpdateEx(sql.toString(), trxName);

		// Apply Restrictions
		StringBuilder whereClause = new StringBuilder(getDeleteSQLWhere(MPPMRP.Table_Name, "mrp." + MPPMRP.COLUMNNAME_PP_MRP_ID + " = "
				+ MPPMRP.COLUMNNAME_PP_MRP_ID, AD_Client_ID, AD_Org_ID, M_Warehouse_ID, S_Resource_ID, M_Product_ID, null));
		...
		where = getDeleteSQLWhere(MRequisitionLine.Table_Name,
		 ...
		 */
		/*
		 * if (isSynchronize()) { // Delete Distribution Order with Draft Status
		 * where = getDeleteSQLWhere(MDDOrderLine.Table_Name,"mrp." +
		 * MPPMRP.COLUMNNAME_DD_OrderLine_ID + " = " +
		 * MPPMRP.COLUMNNAME_DD_OrderLine_ID , AD_Client_ID, AD_Org_ID, null,
		 * null, M_Product_ID , MPPMRP.TYPEMRP_Supply) +
		 ...
		 * myParameters.add(MPPMRP.DOCSTATUS_Drafted);
		 * deletePO(MDDOrderLine.Table_Name, where, trxName
		 * ,myParameters.toArray()); }
		 */
		...
		whereClause = new StringBuilder(getDeleteSQLWhere(MPPMRP.Table_Name, "mrp." + MPPMRP.COLUMNNAME_PP_MRP_ID + " = " + MPPMRP.COLUMNNAME_PP_MRP_ID,
				AD_Client_ID, AD_Org_ID, M_Warehouse_ID, null, M_Product_ID, MPPMRP.TYPEMRP_Supply));
		sql = new StringBuilder();
			...
			public void run(String trxName) {
				...
				try {
					...
					for (int level = 0; level <= lowlevel; level++) {
						...
						sql.append("SELECT mrp.M_Product_ID, mrp.LowLevel, mrp.Qty, mrp.DatePromised")
						...
						while (rs.next()) {
							...
							// if demand is forecast and promised date less than
							// or equal to today, ignore this QtyGrossReq
							if (MPPMRP.TYPEMRP_Demand.equals(TypeMRP) && MPPMRP.ORDERTYPE_Forecast.equals(OrderType) && DatePromised.compareTo(getToday()) <= 0) {
								continue;
							}

							// New Product
							if (product == null || product.getID() != M_Product_ID) {
								// If exist QtyGrossReqs of last Demand
								// verify/calculate plan
								if (QtyGrossReqs.signum() != 0) {
									...
									} else if (MPPProductPlanning.ORDER_POLICY_Lot_For_Lot.equals(m_product_planning.getOrder_Policy())
											&& BeforeDateStartSchedule.compareTo(Planning_Horizon) <= 0) {
										// TODO: Q: when we have this situation
										// because on LFL we balance the Demand
										// imediately
								...
								// Setting MRP Change net Update out the model
								// validator and out transaction
								...
								// If No Product Planning found, go to next MRP
								// record
								if (m_product_planning == null)
									continue;
							...
							// first DatePromised.compareTo for
							...
							if (MPPProductPlanning.ORDER_POLICY_PeriodOrderQuantity.equals(m_product_planning.getOrder_Policy())) {
								// Verify if is DatePromised < DatePromisedTo
								// then Accumulation QtyGrossReqs
							...
							// If Order_Policy = LoteForLote then always create
							// new range for next period and put QtyGrossReqs
							else if (MPPProductPlanning.ORDER_POLICY_Lot_For_Lot.equals(m_product_planning.getOrder_Policy())) {
								QtyGrossReqs = QtyGrossReqs.add(Qty);
						...
						// If exist QtyGrossReq of last Demand after finish
						// while verify plan
	 ...
	 * @throws SQLException
	 */
	private void setProduct(int AD_Client_ID, int AD_Org_ID, int S_Resource_ID, int M_Warehouse_ID, MProduct product, int PP_MRP_ID, String trxName)
			throws SQLException {
		createMRPPegging(trxName);
	...
	protected MPPProductPlanning getProductPlanning(int AD_Client_ID, int AD_Org_ID, int S_Resource_ID, int M_Warehouse_ID, MProduct product, String trxName)
			throws SQLException {
		// Find data product planning demand
		...
		if (pp2.getPlanner_ID() <= 0) {
			pp2.setPlanner_ID(getPlanner_ID() == null ? Env.getAD_User_ID(getCtx()) : getPlanner_ID());
		}
	...
	protected BigDecimal getQtyOnHand(MPPProductPlanning pp, String trxName) {
	 ...
	 *            Demand Date Start Schedule
	 * @throws SQLException
	 */
	private void calculatePlan(int AD_Client_ID, int AD_Org_ID, int M_Warehouse_ID, int PP_MRP_ID, MProduct product, Timestamp DemandDateStartSchedule,
			String trxName) throws SQLException {
		// Set Yield o QtyGrossReqs
		// Note : the variables DemandDateStartSchedule ,
		// DemandDateFinishSchedule are same DatePromised to Demands Sales Order
		// Type

		log.info("Create Plan ...");

		// Check Internal Error: product from data planning should be the same
		// with the product given as argument
		if (m_product_planning.getM_Product_ID() != product.getID()) {
			throw new IllegalStateException("MRP Internal Error:" + " DataPlanningProduct(" + m_product_planning.getM_Product_ID() + ")" + " <> Product("
					+ product + ")");
		}
		...
		if (QtyNetReqs.signum() > 0) { // entire qty is available or scheduled
										// to receipt
		...
		if (QtyPlanned.signum() > 0 && m_product_planning.getOrder_Min().signum() > 0) {
			if (m_product_planning.getOrder_Min().compareTo(QtyPlanned) > 0) {
				String comment = Msg.translate(getCtx(), MPPProductPlanning.COLUMNNAME_Order_Min) + " : " + m_product_planning.getOrder_Min();
		...
		if (QtyPlanned.compareTo(m_product_planning.getOrder_Max()) > 0 && m_product_planning.getOrder_Max().signum() > 0) {
			String comment = Msg.translate(getCtx(), MPPProductPlanning.COLUMNNAME_Order_Max) + " : " + m_product_planning.getOrder_Max();
		...
		// Indicates that there is an unsatisfied material requirement inside
		// the planning time fence for this product.
		// You should either manually schedule and expedite orders to fill this
		// demand or delay fulfillment
		// of the requirement that created the demand.
		if (TimeFence != null && DemandDateStartSchedule.compareTo(TimeFence) < 0) {
			String comment = Msg.translate(getCtx(), MPPProductPlanning.COLUMNNAME_TimeFence) + " : " + m_product_planning.getTimeFence() + "-"
					+ Msg.getMsg(getCtx(), "Date") + " : " + TimeFence + " " + Msg.translate(getCtx(), MPPOrder.COLUMNNAME_DatePromised) + " : "
					+ DemandDateStartSchedule;
			createMRPNote("MRP-100", AD_Org_ID, PP_MRP_ID, product, null, QtyPlanned, comment, trxName);
		}

		// MRP-020 Create
		// Indicates that a supply order should be created to satisfy a negative
		// projected on hand.
	...
	protected void createSupply(int AD_Org_ID, int PP_MRP_ID, MProduct product, BigDecimal QtyPlanned, Timestamp DemandDateStartSchedule, String trxName)
		...
		// Manufacturing Order
			...
			throw new IllegalStateException("MRP Internal Error: Don't know what document to " + "create for " + product + "(" + m_product_planning + ")");
		}
	}

	private void createMRPPegging(String trxName) {
		if (demands == null || demands.size() == 0 || supplies == null || supplies.size() == 0)
			return;

		Iterator<Entry<Integer, BigDecimal>> iteratorDemands = demands.entrySet().iterator();
		Iterator<Entry<Integer, BigDecimal>> iteratorSupplies = supplies.entrySet().iterator();
	...
	private MPPMRP getSupply(String columnMaster, Integer master_id, String columnDetail, Integer detail_id, String trxName) {
		final StringBuilder whereClause = new StringBuilder(MPPMRP.COLUMNNAME_TypeMRP).append("=? AND ").append(columnMaster).append("=? ");
		...
		return new Query(getCtx(), MPPMRP.Table_Name, whereClause.toString(), trxName).setClient_ID().setParameters(parameters).first();
	}
	...
	protected void createPPOrder(int AD_Org_ID, int PP_MRP_ID, MProduct product, BigDecimal QtyPlanned, Timestamp DemandDateStartSchedule, String trxName)
		...
		order.addDescription("Generada por MRP");
	...
	private void deletePO(String tableName, String whereClause, String trxName, Object... params) throws SQLException {
		...
		// class
		POResultSet<PO> rs = new Query(getCtx(), tableName, whereClause, trxName).setParameters(params).scroll();
		try {
			while (rs.hasNext()) {
				rs.next().delete(true);
			}
	 ...
	 * 
	 * @param code
	 *            MRP/DRP Code (see MRP-xxx and DRP-xxx messages)
	 * @param AD_Org_ID
	 *            organization
	 * @param PP_MRP_ID
	 *            MRP record id
	...
	protected void createMRPNote(String code, int AD_Org_ID, int PP_MRP_ID, MProduct product, String documentNo, BigDecimal qty, String comment, String trxName)
		...
		reference = (reference != "" ? reference : reference) + "M_Product_ID" + "->" + product.getID();
	...
	protected void createMRPNote(String code, int AD_Org_ID, int PP_MRP_ID, MProduct product, BigDecimal qty, Timestamp DemandDateStartSchedule, Exception e,
			String trxName) throws SQLException {
	 ...
	 * @param DemandDateStartSchedule
	 * @return Net Requirements: <li>positive qty means entire qty is available
	 *         or scheduled to receipt <li>negative qty means qty net required
	 * @throws SQLException
	 */
	private BigDecimal getNetRequirements(int AD_Client_ID, int AD_Org_ID, int M_Warehouse_ID, MProduct product, Timestamp DemandDateStartSchedule,
			String trxName) throws SQLException {
		BigDecimal QtyNetReqs = QtyProjectOnHand.subtract(QtyGrossReqs);

		final String whereClause =
		// Planning Dimension
		"AD_Client_ID=? AND AD_Org_ID=?" + " AND M_Product_ID=? AND M_Warehouse_ID=?"
		...
		Collection<MPPMRP> mrps = new Query(getCtx(), MPPMRP.Table_Name, whereClause, trxName).setParameters(parameters)
				.setOrderBy(MPPMRP.COLUMNNAME_DateStartSchedule).list();
		for (MPPMRP mrp : mrps) {
			if (mrp.isReleased()) {
			...
			if (DemandDateStartSchedule != null) {
				...
				// Indicates that a schedule supply order is due before it is
				// needed and should be delayed,
				// or demand rescheduled to an earlier date.
				// aka: Push Out
				if (mrp.isReleased() && QtyNetReqs.negate().signum() > 0 && mrp.getDateStartSchedule() != null
				...
				// Indicates that a scheduled supply order is due after is
				// needed and should be rescheduled to
				...
				// Indicate that a supply order should be released. if it is a
				// draft order, it must also be approved.
				...
				// Indicates that a supply order was not released when it was
				// due, and should be either released
				...
				// Indicate that a scheduled supply order is no longer needed
