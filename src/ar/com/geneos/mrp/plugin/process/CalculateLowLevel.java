...
public class CalculateLowLevel extends SvrProcess {
	...
	protected String doIt() throws Exception {
		...
		POResultSet<MProduct> rs = new Query(getCtx(), MProduct.Table_Name, "AD_Client_ID=?", get_TrxName())
				.setParameters(new Object[] { Env.getAD_Client_ID(getCtx()) }).setOrderBy(MUColumnNames.COLUMNNAME_M_Product_ID).scroll();
		rs.setCloseOnError(true);
		while (rs.hasNext()) {
			MProduct product = rs.next();
			try {
				int lowlevel = MPPProductBOMLine.getLowLevel(getCtx(), product.getID(), get_TrxName());
