 ...
 */
public class MPPOrderNode extends LP_PP_Order_Node
{
	...
	private static final String ENTITYTYPE_UserMaintained = "U";
	...
	//@Override
	public LP_PP_Order_Workflow getPP_Order_Workflow()
	{
	...
	private void setDateFinish(boolean override)
	{
		...
		/*
		Timestamp dateFinish = DB.getSQLValueTSEX(get_TrxName(), sql, getID(),
	...
	{
		final String whereClause = LP_PP_Cost_Collector.COLUMNNAME_PP_Order_Node_ID+"=?"
		+" AND "+LP_PP_Cost_Collector.COLUMNNAME_PP_Order_ID+"=?"
		+" AND "+LP_PP_Cost_Collector.COLUMNNAME_DocStatus+" IN (?,?)"
		+" AND "+LP_PP_Cost_Collector.COLUMNNAME_costcollectortype+"=?"
		;
		BigDecimal variance = new Query(getCtx(), LP_PP_Cost_Collector.Table_Name, whereClause, get_TrxName())
		.setParameters(new Object[]{
				getPP_Order_Node_ID(),
				getPP_Order_ID(),
				MPPCostCollector.STATUS_Completed, MPPCostCollector.STATUS_Closed,
				costCollectorType,
		})
		.sum(columnName);
		//
	...
	{
		return getVariance(MPPCostCollector.COSTCOLLECTORTYPE_UsegeVariance,
				MPPCostCollector.COLUMNNAME_SetupTimeReal);
	}
	
	public BigDecimal getDurationUsageVariance()
	{
		return getVariance(MPPCostCollector.COSTCOLLECTORTYPE_UsegeVariance,
				MPPCostCollector.COLUMNNAME_DurationReal);
	}
