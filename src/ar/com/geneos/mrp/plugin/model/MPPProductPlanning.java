 ...
 */
public class MPPProductPlanning extends LP_PP_Product_Planning
{
	/**
	 * 
	 */
	...
	{
		final String sql = "SELECT MIN("+MUColumnNames.COLUMNNAME_S_Resource_ID+")"
							+" FROM "+MResource.Table_Name
							+" WHERE "+MUColumnNames.COLUMNNAME_IsManufacturingResource+"=?"
							+" AND "+MUColumnNames.COLUMNNAME_ManufacturingResourceType+"=?"
							+" AND "+MUColumnNames.COLUMNNAME_M_Warehouse_ID+"=?"; 
	...
	protected boolean beforeSave(boolean newRecord)
	{
		...
		{
			throw new IllegalStateException("@Order_Min@ > @Order_Max@");
		}
		//
		// Check Order_Period
		...
		{
			throw new IllegalStateException("@Order_Period@ <= 0");
		}
		//
		// Check Order_Qty
		...
		{
			throw new IllegalStateException("@Order_Qty@ <= 0");
		}
		//
		return true;
	}
	...
	{
		return new MResource(getCtx(), getS_Resource_ID(), null);
	}
