...
import ar.com.geneos.mrp.plugin.exception.CostInsufficientQtyException;
 ...
 */
public class MCostQueue extends LP_M_CostQueue {
	/**
	 * 
	 */
	...
	public static MCostQueue get(MProduct product, int M_AttributeSetInstance_ID, MAcctSchema as, int AD_Org_ID, int M_CostElement_ID, String trxName) {
		final String whereClause = "AD_Client_ID=? AND AD_Org_ID=?" + " AND " + COLUMNNAME_M_Product_ID + "=?" + " AND " + COLUMNNAME_M_AttributeSetInstance_ID
				+ "=?" + " AND " + COLUMNNAME_M_CostType_ID + "=?" + " AND " + COLUMNNAME_C_AcctSchema_ID + "=?" + " AND " + COLUMNNAME_M_CostElement_ID + "=?";
		final Object[] params = new Object[] { product.getAD_Client_ID(), AD_Org_ID, product.getID(), M_AttributeSetInstance_ID, as.getM_CostType_ID(),
				as.getID(), M_CostElement_ID };
		MCostQueue costQ = new Query(product.getCtx(), Table_Name, whereClause, trxName).setParameters(params).first();
		// New
		if (costQ == null) {
	...
	public static MCostQueue getQueueForAdjustment(MCostDetail cd, MCost cost, String trxName) {
		final MCostElement ce = MUMCostElement.get(cost.getCtx(), cost.getM_CostElement_ID());
		...
		MCostQueue cq = new Query(cost.getCtx(), Table_Name, whereClause.toString(), trxName).setParameters(params)
				.setOrderBy(COLUMNNAME_M_Costqueue_ID + " DESC").first();
		if (cq == null) {
	...
	public static MCostQueue[] getQueue(MCost cost, Timestamp dateAcct, String trxName) {
		final MCostType ct = new MCostType(cost.getCtx(), cost.getM_CostType_ID(), cost.get_TrxName());
		final MCostElement ce = MUMCostElement.get(cost.getCtx(), cost.getM_CostElement_ID());
		...
		/*
		 * else if (ce.isLandedCost()) { orderByFlag = " ASC"; }
		 */
		if (orderByFlag == null) {
			throw new IllegalArgumentException("Cost element should be FIFO or LIFO - " + ce + ", CostingMethod=" + ct.getCostingMethod());
		}
		String orderBy = COLUMNNAME_DateAcct + orderByFlag + "," + COLUMNNAME_M_Costqueue_ID + orderByFlag;
		//
		List<Object> params = new ArrayList<Object>();
		StringBuffer whereClause = new StringBuffer();
		...
		List<MCostQueue> list = new Query(cost.getCtx(), Table_Name, whereClause.toString(), trxName).setParameters(params).setOrderBy(orderBy).list();
		//
	...
	public static MCostQueue[] getQueue(MProduct product, int M_ASI_ID, MAcctSchema as, int Org_ID, MCostElement ce, String trxName) {
		...
		String sql = "SELECT * FROM M_CostQueue " + "WHERE AD_Client_ID=? AND AD_Org_ID=?" + " AND M_Product_ID=?"
				+ " AND M_CostType_ID=? AND C_AcctSchema_ID=?" + " AND M_CostElement_ID=?";
		if (M_ASI_ID != 0)
			sql += " AND M_AttributeSetInstance_ID=?";
		sql += " AND CurrentQty<>0 " + "ORDER BY M_AttributeSetInstance_ID ";
		if (!MUMCostElement.isFifo(ce))
			sql += "DESC";
	 ...
	 * @deprecated
	 */
	public static BigDecimal getCosts(MProduct product, int M_ASI_ID, MAcctSchema as, int Org_ID, int M_Warehouse_ID, MCostElement ce, BigDecimal Qty,
			String trxName) {
		if (Qty.signum() == 0)
			return Env.ZERO;
	...
	public static List<CostComponent> getCostLayers(MCost cost, BigDecimal Qty, Timestamp dateAcct, String trxName) {
		...
		for (int i = 0; i < costQ.length; i++) {
			...
			// Negative Qty i.e. add
			/*
			 * if (remainingQty.signum() <= 0 && queue.getCurrentQty().signum()
			 * != 0) { BigDecimal reduction = remainingQty;//anca if
			 * (reduction.negate().compareTo(queue.getCurrentQty()) > 0)//anca
			 * reduction = queue.getCurrentQty().negate();//anca lastPrice =
			 * queue.getCurrentCostPrice(); BigDecimal costBatch =
			 * lastPrice.multiply(reduction); list.add(new
			 * CostComponent(reduction, lastPrice)); cost = cost.add(costBatch);
			 * s_log.config("ASI=" + queue.getM_AttributeSetInstance_ID() +
			 * " - Cost=" + lastPrice + " * Qty=" + reduction+ "(!) = " +
			 * costBatch); //return cost; remainingQty =
			 * remainingQty.subtract(reduction);//anca if
			 * (queue.getCurrentQty().equals(Env.ZERO)) { return list; }
			 * 
			 * }
			 */
			// Positive queue
		...
		if (lastPrice == null) {
			lastPrice = MCost.getSeedCosts(MProduct.get(cost.getCtx(), cost.getM_Product_ID()), cost.getM_AttributeSetInstance_ID(),
					MAcctSchema.get(cost.getCtx(), cost.getC_AcctSchema_ID()), cost.getAD_Org_ID(), cost.getM_Warehouse_ID(), cost.getCostingMethod(), 0 // C_OrderLine_ID
					);
			if (lastPrice == null) {
				s_log.info("No Price found");
				return null;
	...
	public String toString() {
		return getClass().getSimpleName() + "[" + getID() + ", AD_Org_ID=" + getAD_Org_ID() + ", M_Product_ID=" + getM_Product_ID() + ", M_ASI_ID="
				+ getM_AttributeSetInstance_ID() + ", Price=" + getCurrentCostPrice() + ", Qty=" + getCurrentQty() + ", DateAcct=" + getDateAcct() + "]";
	}
	 ...
	 * @category arhipac
	 */
	public static MCostQueue add(MProduct product, int M_AttributeSetInstance_ID, MAcctSchema as, int AD_Org_ID, int M_CostElement_ID, BigDecimal amt,
			BigDecimal qty, int precision, MCostDetail cd, String trxName) {
		if (CLogMgt.isLevelFine())
			s_log.fine("Entering: org=" + AD_Org_ID + ", product=" + product.getID() + ", asi=" + M_AttributeSetInstance_ID + ", amt=" + amt + ", qty=" + qty
					+ ", precision=" + precision);
		//
