...
public class MCostQueue extends LP_M_CostQueue
{
		...
		MAcctSchema as, int AD_Org_ID, int M_CostElement_ID, String trxName)
	{
		...
		final Object[] params = new Object[]{product.getAD_Client_ID(), AD_Org_ID,
											product.getID(), M_AttributeSetInstance_ID,
											as.getM_CostType_ID(), as.getID(),
											M_CostElement_ID};
	...
	public static MCostQueue[] getQueue (MCost cost, Timestamp dateAcct, String trxName)
	{
		...
		if (MUMCostType.getCostingMethod(ct) != null)
		{
			if (MCostElement.COSTINGMETHOD_Fifo.equals(MUMCostType.getCostingMethod(ct)))
				orderByFlag = " ASC";
			else if (MCostElement.COSTINGMETHOD_Lifo.equals(MUMCostType.getCostingMethod(ct)))
				orderByFlag = " DESC";
		}
		else if (ce != null && MUMCostType.isFifo(ct))
		{
			orderByFlag = " ASC";
		}
		else if (ce != null && MUMCostType.isLifo(ct))
		{
			orderByFlag = " DESC";
		}
		...
		if (orderByFlag == null)
		{
			throw new IllegalArgumentException("Cost element should be FIFO or LIFO - "+ce+", CostingMethod="+MUMCostType.getCostingMethod(ct));
		}
	...
	public static MCostQueue add (MProduct product, int M_AttributeSetInstance_ID,
		...
		String trxName)
	{
		if(CLogMgt.isLevelFine()) s_log.fine("Entering: org=" + AD_Org_ID + ", product=" + product.getID() + ", asi=" + M_AttributeSetInstance_ID + ", amt=" + amt + ", qty=" + qty + ", precision=" + precision);
		//
