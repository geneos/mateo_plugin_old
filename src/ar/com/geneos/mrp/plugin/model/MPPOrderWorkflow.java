 ...
 */
public class MPPOrderWorkflow extends LP_PP_Order_Workflow {
	...
	public MPPOrderWorkflow(MWorkflow workflow, int PP_Order_ID, String trxName) {
		...
		/*
		 * Migración Libero diferencias de tipos entre cost de workflow y de
		 * order workflow Analizar impacto
		 * 
		 * @autor pepo
		 */

		setCost(new BigDecimal(workflow.getCost()));
	 ...
	 *            ID
	 * @param AD_Client_ID
	 *            if >= 0 the node is checked if is valid for this tenant
	...
	private MPPOrderNode[] getNodesInOrder(int AD_Client_ID) {
		ArrayList<MPPOrderNode> list = new ArrayList<MPPOrderNode>();
		addNodesSF(list, getPP_Order_Node_ID(), AD_Client_ID); // start with
																// first
		// Remaining Nodes
	 ...
	 * 
	 * @param activity
	 *            current activity / reporting activity
	 * @param movementDate
	 * @param milestone
	 *            stop on previous milestone
	 */
	public void closeActivities(MPPOrderNode activity, Timestamp movementDate, boolean milestone) {
		if (activity.getPP_Order_Workflow_ID() != getID()) {
			throw new IllegalStateException("Activity and Order Workflow not matching " + " (" + activity + ", PP_Order_Workflow_ID=" + getID() + ")");
		}
		MPPOrder order = getMPPOrder();

		for (int nodeId = activity.getID(); nodeId != 0; nodeId = getPrevious(nodeId, getAD_Client_ID())) {
			MPPOrderNode node = getNode(nodeId);
			// Break the cycle
			if (milestone && node.isMilestone() && node.getID() != activity.getID()) {
				break;
			}
			if (MPPOrderNode.DOCSTATUS_Drafted.equals(node.getDocStatus())) {
				...
				// arhipac: cristi_pup: Generate even if nothing was reporting
				// on this activity
				...
				node.load(order.get_TrxName()); // reload because it was
												// modified outside of our
												// context
				node.closeIt();
				node.save();
