...
public class AveragePOCostingMethod extends AbstractCostingMethod implements ICostingMethod {
	...
	public void calculate() {
		...
		lastCostDetail = MCostDetail.getLastTransaction(model, transaction, accountSchema.getC_AcctSchema_ID(), dimension.getM_CostType_ID(),
				dimension.getM_CostElement_ID(), model.getDateAcct(), costingLevel);
		...
		if (transaction.getMovementType().endsWith("+")) {
			...
			if (quantityOnHand.signum() != 0 && getNewAccumulatedAmount(lastCostDetail).signum() == 0 && costThisLevel.signum() != 0) {
				adjustCost = quantityOnHand.add(transaction.getMovementQty()).multiply(costThisLevel)
						.subtract(costThisLevel.multiply(transaction.getMovementQty()));
	...
	public void adjustCostDetail(MCostDetail costDetail) {
		...
		if (MTransaction.MOVEMENTTYPE_VendorReceipts.equals(transaction.getMovementType())) {
			...
			CostEngineFactory.getCostEngine(clientId).createCostDetail(accountSchema, costType, costElement, transaction, MUMTransaction.getDocumentLine(transaction), true);
	}
