...
public class LastInvoiceCostingMethod extends AbstractCostingMethod implements ICostingMethod {
		...
		if (costDetail.getC_OrderLine_ID() != 0) {
			if (!isReturnTrx) {
				if (costDetail.getQty().signum() != 0)
					...
					BigDecimal cCosts = cost.getCurrentCostPrice().add(costDetail.getAmt());
					cost.setCurrentCostPrice(cCosts);
				}
			}
			cost.add(costDetail.getAmt(), costDetail.getQty());
			s_log.finer("Inv - LastInv - " + cost);
		} else if (costDetail.getM_InOutLine_ID() != 0 // AR Shipment Detail
														// Record
				|| costDetail.getM_MovementLine_ID() != 0 || costDetail.getM_InventoryLine_ID() != 0
	...
	@Override
	public java.math.BigDecimal getNewAccumulatedAmount(MCostDetail cd) {
		return null;
	}

	@Override
	public java.math.BigDecimal getNewCurrentCostPriceLowerLevel(MCostDetail cd, int scale, int roundingMode) {
		return null;
	}

	@Override
	public java.math.BigDecimal getNewAccumulatedAmountLowerLevel(MCostDetail cd) {
		return null;
	}

	@Override
	public java.math.BigDecimal getNewAccumulatedQuantity(MCostDetail cd) {
		return null;
	}
